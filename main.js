   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("𝘰𝘰𝘱𝘴 𝘢𝘥𝘢 𝘬𝘦𝘴𝘢𝘭𝘢𝘩𝘢𝘯 𝘢𝘱𝘢𝘬𝘢𝘩 𝘬𝘢𝘮𝘶 𝘴𝘶𝘥𝘢𝘩 𝘣𝘦𝘯𝘢𝘳 𝘮𝘦𝘮𝘢𝘴𝘶𝘬𝘢𝘯 𝘯𝘰𝘮𝘰𝘳𝘯𝘺𝘢, 𝘯𝘰𝘮𝘰𝘳 𝘩𝘢𝘳𝘶𝘴 𝘥𝘪 𝘢𝘸𝘢𝘭𝘪 +62𝘹𝘹𝘹𝘹𝘹")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.cyanBright(`𝙼𝙰𝚂𝚄𝙺𝙰𝙽 𝙽𝙾𝙼𝙾𝚁 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿 𝙼𝚄 🥳\n[ 𝚁𝙴𝚈𝚉 ] : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
      ...
      setTimeout(async () => {
         ...
         console.log(chalk.black(chalk.bgGreen(`𝙽𝙸 𝙺𝙾𝙳𝙴 𝙿𝙰𝙸𝚁𝙸𝙽𝙶 𝙻𝚄 :`)), chalk.black(chalk.white(code)))
      }, 3000)
   }
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            ...
            console.log(chalk.yellow(`🎉 𝙱𝙴𝚁𝙷𝙰𝚂𝙸𝙻 𝚃𝙴𝚁𝙷𝚄𝙱𝚄𝙽𝙶 𝙺𝙴 => ` + JSON.stringify(XeonBotInc.user, null, 2)))
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ 𝙱𝙾𝚃 𝙱𝚈 𝚁𝙴𝚈𝚉 ]`)}\n\n`))
            console.log(chalk.cyan(`<<—————————————————————————————————————————————————>>`))
	        console.log(chalk.red(`\n${themeemoji} YT CHANNEL : @Litzz445`))
            console.log(chalk.yellow(`${themeemoji} GITHUB : Retbot1238 `))
            console.log(chalk.red(`${themeemoji} INSTAGRAM : @reymwmwk112 `))
            console.log(chalk.yellow(`${themeemoji} WA NUMBER : 6283181666350`))
            console.log(chalk.red(`${themeemoji} CREDIT : DgXeon And Reyz\n`))
